...
# ---------- App ----------
app = FastAPI(title="Dynamic Persona Generator with LLM", version="0.2")
...
class PersonaChatReq(BaseModel):
    ...
    llm_provider: Optional[str] = "rule_based"  # "openai", "anthropic", "google", "rule_based"
...
class LLMIntegration:
    def __init__(self, provider: str = "openai"):
        ...
        if provider != "rule_based" and not self.api_key:
            logger.warning(f"No API key found for {provider}, falling back to rule-based responses")
    ...
    def generate_persona_response(self, persona_traits: Dict, chat_personality: str, 
        ...
        """Generate persona response using LLM or rule-based fallback"""
        
        if self.provider == "rule_based":
            return self._rule_based_response(persona_traits, message)
    ...
    def _rule_based_response(self, persona_traits: Dict, message: str) -> str:
...
Respond naturally as this persona would, using casual language and expressing your genuine concerns and interests. Keep responses conversational and authentic to your persona. Limit response to 1-2 sentences."""
...
@app.get("/health")
def health():
    return {"ok": True, "users": int(len(USERS)), "llm_providers": ["rule_based", "openai", "anthropic", "google"]}
...
def chat_with_persona(persona_id: str, req: PersonaChatReq):
    """Chat with a specific persona using LLM or rule-based responses"""
    try:
        cluster_id = int(persona_id.split('_')[1])
    except:
        raise HTTPException(status_code=400, detail="Invalid persona ID")
    
    # Get persona traits (regenerate for now)
    ...
    llm = LLMIntegration(req.llm_provider)
